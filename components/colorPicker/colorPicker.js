...
xComponent({
  ...
  data: {
    initColor: 'rgb(255, 26, 23)',  // 红色
    flag: true
  },
  async created() {
    const { initColor } = this.$getItem('maincolor') || this.data;
    this.setData({
      hueColor: this.hsv2rgb((this.rgb2hsv(initColor)).h, 100, 100)
    })
  },
  async mounted() {
    const { h, s, v } = this.rgb2hsv(this.data.initColor)
    let option = {
      hsv: {
        h, s, v
      },
      colorRes: this.data.initColor
    }
    if (this.properties.movable) {
      const rect = await this.$getRect('.colorpicker', true)
      if (rect) {
        this.SV = {
        ...
        option = Object.assign({}, option, { // 初始化定位
          x: Math.round(s * this.SV.Step),
          y: Math.round((100 - v) * this.SV.Step)
        })
      }
    }
    this.$set(Object.assign(option))
  },
  methods: {
    onChangeMovableView(e) {
      let {
        x,
        y
      } = e.detail;
